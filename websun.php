...
# 2010-2018 (c)
    ...
    0.2.2 - added namespace, added static helpers (by @KarelWintersky)

    0.2.1 - comparsion returns false when any of the operands is null (i.e. undefined variable)
...
class websun
{
    ...
    public function parse_template($template)
    {
        ...
        if ($this->profiling AND !$this->predecessor) {
            $this->TIMES['_TOTAL'] = round(microtime(true) - $start, 4) . " s";
            // ksort($this->TIMES);
    ...
    function var_value($string)
    {
        ...
        if ($this->profiling)
            $this->write_time(__FUNCTION__, $start, microtime(true));
    ...
    function parse_vars_templates_functions($matches)
    {
        ...
        if (mb_substr($work, 0, 1) == '@') { // функции {* @name(arg1,arg2) | template *}
            ...
            if (preg_match($p, mb_substr($work, 1), $m)) {
                ...
                $call = null; // Сюда запишем некоторые параметры того, что будем вызывать
                ...
                if (!$w) { // "звёздочек" нет - простая функция или статический метод
                    ...
                    // "звёздочки" есть - нужно получить из переменной
